...
<head>
    ...
    <title>Admin - Edit Tipe Kamar</title>
    <style>
        :root {
            color-scheme: dark;
            --bg: #0b1120;
    ...
    <div class="layout">
        <aside class="sidebar">
            ...
            <ul class="nav-list">
                ...
                <li class="nav-item"><a href="/admin-logout"><span class="nav-icon"><i data-feather="log-out"></i> data-feather="log-out"></i></span> Logout</a></li>
            </ul>
        </aside>
        <main class="main">
            <section class="topbar">
                <div>
                    <h1>Admin - Edit Tipe Kamar</h1>
                    <p>Perbarui data tipe kamar sesuai kebutuhan.</p>
                </div>
                <div class="badges"><span class="badge">Tipe Kamar</span></div>
            </section>
            <div class="form-card">
                <h2 class="section-title">Form Edit Tipe Kamar</h2>
                @if($errors->any())
                    <div class="alert alert-error"><ul style="margin:0; padding-left:18px;">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></div>
                @endif
                <form method="POST" action="/admin/tipe-kamar/{{ $data->id_tipe }}">
                    @csrf
                    @method('PUT')
                    <div class="input-group"><label for="tipe_kamar">Tipe Kamar</label><input id="tipe_kamar" name="tipe_kamar" value="{{ old('tipe_kamar', $data->tipe_kamar) }}" required></div>
                    <div class="input-group"><label for="harga">Harga</label><input id="harga" name="harga" value="{{ old('harga', $data->harga) }}" required></div>
                    <div style="display:flex; gap:.85rem; flex-wrap:wrap; margin-top:1rem;"><button type="submit" class="btn btn-primary">Simpan</button><a href="/admin/tipe-kamar" class="btn btn-secondary">Batal</a></div>
                </form>
            </div>
        </main>
    </div>
