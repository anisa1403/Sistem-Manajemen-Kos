...
files = {
    'resources/views/tipe_kamar/index.blade.php': '''<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="layout">
        <aside class="sidebar">
            ...
            <ul class="nav-list">
                <li class="nav-item"><a href="/admin/dashboard"><span class="nav-icon">🏠</span> Dashboard</a></li>
                <li class="nav-item"><a href="/admin/user"><span class="nav-icon">👥</span> Pengguna</a></li>
                <li class="nav-item"><a href="/tipe-kamar"><span class="nav-icon">🏷️</span> Tipe Kamar</a></li>
                <li class="nav-item"><a href="/kamar"><span class="nav-icon">🚪</span> Kamar</a></li>
                <li class="nav-item"><a href="/sewa"><span class="nav-icon">📄</span> Sewa</a></li>
                <li class="nav-item"><a href="/pembayaran"><span class="nav-icon">💳</span> Pembayaran</a></li>
                <li class="nav-item"><a href="/admin-logout"><span class="nav-icon">🚪</span> Logout</a></li>
            </ul>
        </aside>
        <main class="main">
            ...
            <div class="table-card">
                ...
                <div style="display:flex; justify-content:space-between; flex-wrap:wrap; gap:0.75rem; align-items:center; margin-bottom:1rem;">
                    <div style="color: rgba(226,232,240,.78);">Total tipe kamar: <strong>{{ $tipe_kamar->count() }}</strong></div>
                    <a href="/tipe-kamar/create" class="btn btn-primary">Tambah Tipe Kamar</a>
                </div>
                <div style="overflow-x:auto;">
                    <table class="table">
                        ...
                        <tbody>
                            @forelse($tipe_kamar as $tipe)
                                <tr>
                                    ...
                                    <td style="display:flex; gap:.65rem; flex-wrap:wrap;">
                                        <a href="/tipe-kamar/{{ $tipe->id_tipe }}/edit" class="btn btn-secondary">Edit</a>
                                        <form method="POST" action="/tipe-kamar/{{ $tipe->id_tipe }}" onsubmit="return confirm('Hapus tipe kamar ini?')" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                            ...
                            @empty
                                <tr><td colspan="5" style="padding:24px; color:var(--muted);">Belum ada tipe kamar.</td></tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
